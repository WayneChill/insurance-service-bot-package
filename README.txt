...
新增銷售                 新增銷售追蹤（對話框引導）
新增增員                 新增增員追蹤（對話框引導）
更新新件 ID 階段         例：更新新件 N001 發單中
更新銷售 ID 階段         例：更新銷售 B001 建議書
更新準增 ID 階段         例：更新準增 R001 約聊聊
行程                     今日行程
本周行程                 本周行程列表
本月行程                 本月行程列表
新增行程                 新增行程（對話框引導，日期用民國7碼如1150331）
新增卡片                 新增信用卡（對話框引導）
刪除卡片                 刪除信用卡（對話框引導）
指令                     顯示指令說明卡片
使用說明                 開啟完整使用說明網頁
...
目前版本：v1.6.0（2026/04/23 發布）

更新內容：
v1.6.0 - 扣款失敗追蹤功能（2026/04/23）

  → 新增「扣款失敗」指令：顯示 Google Sheets 扣款失敗工作表中未完成的記錄

  → 每筆顯示：要保人、公司、保單號碼、類別（轉帳未成功/信用卡請款失敗）、轉帳日、保費

  → 卡片底部含狀態按鈕（待處理/已聯絡/已送出/已完成）及紀錄按鈕

  → 點紀錄可輸入備註，格式：記錄 P001 備註內容

  → 早報新增「保費區」顯示各狀態件數（待處理/已聯絡/已送出）
...
v1.5.0 - 行程功能與介面優化（2026/03/31）
  → 新增行程功能：行程（今日）、本周行程、本月行程、新增行程
  → 新增行程可刪除（卡片上直接點刪除按鈕）
  → 日期輸入支援民國7碼（如 1150331）、時間支援4碼（如 1400）
  → 早報新增行程區（今日/本周/本月筆數），壽險區移至產險區前方
  → 新增新件追蹤功能（新件、新增新件、更新新件）
  → 指令說明新增「新增保服」按鈕，重新排序所有指令
  → 增員追蹤名稱從「準增」改回「增員」
  → 所有卡片字體縮小一碼，指令說明卡片字體維持原本大小

v1.4.0 - 名稱修正與說明優化（2026/03/30）
  → 「增員追蹤」全面更名為「準增追蹤」
  → 指令「增員」改為「準增」，「新增增員」改為「新增準增」，「更新增員」改為「更新準增」
  → 說明選單新增「使用說明」指令（開啟完整說明網頁）
  → 說明選單「待辦」說明更新為「顯示所有待辦彙整」
  → 說明卡片範例改為通用佔位符（查詢 姓名、進度 姓名）
  → 修正使用說明網頁（guide.html）範例個資，改為虛構佔位符

v1.3.0 - 功能修正與優化（2026/03/29）
  → 查詢客戶卡片新增被保人生日顯示（身分證字號下方）
  → 查詢客戶只顯示狀態為「正常」的保單，過濾未承保等無效保單
  → 修正壽星偵測欄位索引，早報壽星計數正確
  → 保服案件初始狀態改為「已聯絡」，與按鈕選項一致
  → 保服案件建立時間只顯示日期，不顯示時分
  → 修正早報不自動發送問題（服務啟動時即初始化排程器）
  → 早報保服區移除「待處理」項目，與按鈕狀態一致
